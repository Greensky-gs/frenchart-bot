...
export class VoiceManager {
    ...
    public addTime(userId: string, time: number) {
        ...
        query(`INSERT INTO voice (user_id, total) VALUES ("${userId}", "${this._cache.get(userId)}") ON DUPLICATE KEY UPDATE total='${this._cache.get(userId)}'`);
    ...
    private async checkDb() {
        await query("CREATE TABLE IF NOT EXISTS voice ( user_id VARCHAR(255) NOT NULL PRIMARY KEY, total INTEGER(255) NOT NULL DEFAULT '0' ) ")
        return true;
    }
    private async fetch() {
