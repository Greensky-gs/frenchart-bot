        ...
        if (!this._winner) {
            this.message
                .edit({
                    components: [],
                    embeds: [
                        classic(this._hoster)
                            .setTitle('Pas de gagnant')
                            .setDescription(`Personne n'a appuyé sur le bouton`)
                            .setColor(this._channel.guild.members.me.displayHexColor ?? 'Orange')
                    ]
                })
                .catch(log4js.trace);
            return;
                ...
                .edit({
                    ...
                    embeds: [
                        classic(this._hoster)
                            .setTitle('Drop')
                            .setDescription(
                                `${pingUser(this._winner)} a gagné **${this._points.toLocaleString()}** points !`
                            )
                            .setColor('#00ff00')
                    ]
                })
    ...
    private get embed() {
        const embed = classic(this._hoster)
            ...
            .setDescription(
                `${pingUser(
                    this._hoster
                )} offre **${this._points.toLocaleString()}** points ! Appuyez sur le bouton pour les obtenir !\nFin <t:${Math.floor(
                    this.endsAt / 1000
                )}:R>`
            )
            .setColor(
                this.endsAt - Date.now() >= 10000
                    ? '#ff0000'
                    : this._channel.guild.members.me.displayHexColor ?? 'Orange'
            );
        return embed;
    }
    private async start() {
        const msg = await this._channel
            .send({
                embeds: [this.embed],
                components: [
                    row(
        ...
        if (!msg)
            return this._channel
                .send(content('msg', `Le drop n'a pas pu être lancé dans ${pingChannel(this._channel)}`))
                .catch(log4js.trace);
