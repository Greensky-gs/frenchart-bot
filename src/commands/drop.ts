...
export default new AmethystCommand({
    ...
    const msg = await message.reply(content('msg', `Dans quel salon voulez-vous lancer le drop ? Répondez par une mention, un nom ou un identifiant. Répondez par \`cancel\` pour annuler`)).catch(log4js.trace);
    ...
    collector.on('collect', async (inp) => {
        ...
        if (step === 'channel') {
            const channel = inp.mentions.channels.first() || await message.guild.channels.cache.get(inp.content) || message.guild.channels.cache.find(x => x.name.toLowerCase().includes(inp.content));
            if (!channel) {
                msg.edit({
