...
export default new AmethystCommand({
    ...
    if (['ajouter', 'add', 'plus'].includes(subcommand)) {
        ...
        if (staffs.cache.length === 0)
            return message.reply(content('msg', `:x: | Il n'y a aucun staff dans la liste`)).catch(log4js.trace);
        message
            .reply(
                content(
                    'msg',
                    classic(message.author)
                        ...
                        .setDescription(
                            `Voici la liste des staffs ( ${staffs.cache.length} ) :\n${staffs.cache
                                .map(pingUser)
                                .join('\n')}`
                        )
                        .setColor('Orange')
