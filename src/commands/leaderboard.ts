...
export default new AmethystCommand({
    ...
    if (leaderboard.length < 10) {
        ...
        new Paginator({
            ...
            displayPages: 'footer',
            interaction,
            interactionNotAllowedContent: content('ctx', { ephemeral: true }, ":x: | Vous n'êtes pas autorisé à interagir avec ce message"),
            invalidPageContent: (max) => ({ content: `:x: | Veuillez choisir un nombre entre **1** et **${max.toLocaleString()}**`, ephemeral: true }),
            modal: {
                title: "Page",
