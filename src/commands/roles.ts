...
export default new AmethystCommand({
    ...
    if (['ajouter', 'add', 'a'].includes(subcommand)) {
        ...
        if (roles.isRoleIncluded(role.id)) {
            const msg = await message.reply(content('msg', `:x: | Le rôle **${role.name}** est déjà un rôle de points. Voulez-vous modifier sa valeur ?`, yesNoRow())).catch(log4js.trace) as Message<true>
            if (!msg) {
                msg.edit({
                    components: []
                }).catch(log4js.trace);
        ...
        if (rs === 'unknown role') return message.reply({
        ...
        if (!roles.isRoleIncluded(role.id)) return message.reply(`:x: | Le rôle **${role.name}** n'est pas un rôle de points`).catch(log4js.trace);
        ...
        if (roles.roles.length === 0) return message.reply(content('msg', `:x: | Aucun rôle n'est définit comme un rôle de points`))
