...
export default new AmethystCommand({
    ...
    if (!inventory || inventory.items.length === 0) return interaction.reply(`Vous n'avez rien dans votre inventaire`).catch(log4js.trace);

    const reduce = (array: item[]) => {
        const items: (item & { count: number })[] = [];
        const findItem = (template: item) => {
            return items.find(x => x.name === template.name && x.quantity === template.quantity && x.price === template.price && x.role_id === template.role_id && x.type === template.type);
        }
        array.forEach((i) => {
            if (!findItem(i)) {
                items.push({
    ...
    const addField = (embed: EmbedBuilder, item: item & { count: number }) => {
        embed.addFields({
            ...
            value: `${item.name} acheté pour ${item.price.toLocaleString()} points${item.type === 'role' ? `\n> ${pingRole(item.role_id)}`:''}`,
            inline: true
        })
    ...
    if (items.length <= 6) {
        ...
        new Paginator({
            ...
            invalidPageContent: (max) => ({ content: `Veuillez choisir un numéro de page entre **1** et **${max.toLocaleString()}**`, ephemeral: true }),
            interaction,
