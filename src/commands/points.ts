...
export default new AmethystCommand({
    ...
    if (['ajouter', 'add', 'plus', '+', 'aj'].includes(subcommand)) {
        ...
        message.reply(content('msg', `✅ | **${points.toLocaleString()}** point${points === 1 ? ' a' : 's ont'} été ajouté${points === 1 ? '' : 's'} à **${user.username}**`)).catch(log4js.trace);
        ...
        if (rs === 'not enough coins') return message.reply(`:x: | **${user.username}** n'a pas assez de points pour en enlever \`${points.toLocaleString()}\``).catch(log4js.trace);

        message.reply(`✅ | **${points.toLocaleString()}** point${points === 1 ? ' a' : 's ont'} été ajouté${points === 1 ? '' : 's'} à **${user.username}**`).catch(log4js.trace);
    } else if (['voir', 'stats', 'view', 'show', 'display', 'disp'].includes(subcommand)) {
        const stats = coins.getData({
            user_id: user.id
        });
        message.reply(content('msg', classic(message.author)
            .setTitle("Points")
            .setDescription(`<@${user.id}> a **${stats.coins.toLocaleString()}** point${stats.coins === 1 ? '' : 's'}`)
